 ...
 * 
 */
@EActivity(R.layout.activity_nrc_add_agent_cert)
public class NrcAddAgentCertActivity extends BaseActivity {

	/**
	 * 标题
	 */
	@ViewById(R.id.nrc_aac_title)
	protected TextView titleTV;

	/**
	 * 证件类型提示
	 */
	@ViewById(R.id.nrc_aac_tip)
	protected TextView tipIV;

	/**
	 * 代理人证件列表
	 */
	@ViewById(R.id.nrc_aac_cert_list)
	protected NoScrollGridView certGridView;

	/**
	 * 授权委托书列表
	 */
	@ViewById(R.id.nrc_aac_license_list)
	...
	protected void onAfterView() {
		...
		tipIV.setText(NrcUtils.AGENT_CERT_TIP);
		initData();
	}
	 ...
	 * 点击 代理人 返回
	 */
	@Click(R.id.nrc_aac_back)
	protected void clickBack() {
		NrcAddAgentCertActivity.this.finish();
	}

	/**
	 * 点击 代理人 保存
	 */
	@Click(R.id.nrc_aac_save)
	...
	private void initData() {
		...
		if (null != localCertList && localCertList.size() > 0) {
			for (TLayySscl cert : localCertList) {
				if (NrcUtils.AGENT_CERT_TIP.equals(cert.getCName())) {
					agentCert = cert;
				} else {
		...
		if (null == agentCert) {
			agentCert = initAgentCert(NrcUtils.AGENT_CERT_TIP);
		}
	 ...
	 * @param direction
	 */
	private void buildDlrCertFj(String filePath) {
		ArrayList<TLayySsclFj> newCertFjList = certFjListMap.get(agentCert.getCId());
		if (null == newCertFjList || newCertFjList.size() == 0) {
			newCertFjList = new ArrayList<TLayySsclFj>();
			certFjListMap.put(agentCert.getCId(), newCertFjList);
			...
			newCertFj.setCSsclId(agentCert.getCId());
	...
	private boolean checkAgentCertData() {
		if (null == agentCert) {
			Toast.makeText(this, "请上传" + NrcUtils.AGENT_CERT_TIP, Toast.LENGTH_SHORT).show();
			return false;
		}
